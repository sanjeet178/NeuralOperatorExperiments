...
def loadDataDarcy(fileName:str):
    ...
    # extract the shapes
    nBatch = xTrainCoordLess.shape[0]
    coordOneDim = xTrainCoordLess.shape[2]
    coordTwoDim = xTrainCoordLess.shape[3]

    # Add coordinate channels to xTrainCoordLess
    coordOne = torch.linspace(0, 1, steps=coordOneDim)
    coordTwo = torch.linspace(0, 1, steps=coordTwoDim)
    gridCoordOne, gridCoordTwo = torch.meshgrid(coordOne, coordTwo, indexing='ij')
    channelCoordOne = gridCoordOne.unsqueeze(0).repeat(nBatch, 1, 1, 1)
    channelCoordTwo = gridCoordTwo.unsqueeze(0).repeat(nBatch, 1, 1, 1)
